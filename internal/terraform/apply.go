...
func New(path string) *TfExec {
	return &TfExec{
		execPath: path,
		opts: Options{
			KenbenchToken:     viper.GetString("TOKEN"),
			KenbenchURI:       viper.GetString("URI"),
...
func (tf *TfExec) enrichVars(vars map[string]string) map[string]string {
	...
	vars["kenbench_token"] = tf.opts.KenbenchToken
	vars["kenbench_uri"] = tf.opts.KenbenchURI
	return vars
}
...
func (tf *TfExec) enrichEnv(env map[string]string) map[string]string {
	...
	env["KENBENCH_TOKEN"] = tf.opts.KenbenchToken
	env["KENBENCH_URI"] = tf.opts.KenbenchURI
